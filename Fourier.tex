...
$U_\sim:$ Wechselanteil \quad $U_0$: Gleichanteil, Mittelwert
 	\item Effektivwert mit \textbf{Fourier-Reihe}:\\
		  $$ U_{\mathit{eff}} = \sqrt{\sum_{k=-\infty}^{\infty} \left|c_k\right|^2} = \sqrt{\sum_{k=0}^{\infty}U_{k,\mathit{eff}}^2} = \sqrt{U_0^2 + \frac{1}{2} \sum_{k=1}^{\infty} \hat{u}_k^2}$$
...
{\small Gleiche Frequenz, aber Phasenverschiebung zwischen $U$ und $I$.}
	      ...
	      \begin{mdframed}[style=exercise,frametitle=Verzerrungsblindleistung $D$]
		      ...
		      \]
			\textcolor{red}{\small{nur g\"ultig f\"ur sinusf\"ormige Spannung $U$ und nicht-sinusf\"ormigen Strom $I$.\\}}
...
\begin{align*}
	U(\omega) &= 2\pi \cdot \sum_{k=-\infty}^{+\infty} \underline{c}_k \cdot \delta(\omega-k\omega_p)
\end{align*}
	\item Übergang FT $\rightarrow$ FR, Komplexe Fourierkoeffizienten, periodische Fortsetzung (vgl. Kap. \ref{komplexe_fr}):
	\[ \boxed{
		\quad \underline{c}_k =
		\frac{1}{t_p} \cdot \underline{X}(k\omega_1) } =
		\frac{1}{t_p}\int_0^{t_p} x(t) \cdot e^{-j\omega_1 t} dt
	\]
	\small{$t_p$: Periodizität, Periodendauer.}\\
	...
	\item Spektrum periodischer Signale:
	\begin{align*}
	U_p(\omega)& = 2\pi \cdot \sum_{k=-\infty}^{+\infty} \frac{\underline{U}(\omega)}{t_p} \cdot \delta(\omega-k\omega_p)\\
	&= \omega_p \cdot \sum_{k=-\infty}^{+\infty} \underline{U}(\omega) \cdot \delta(\omega-k\omega_p)
	\end{align*}

	\item Abtastfrequenz \(\omega_p\):
	\begin{align*}
